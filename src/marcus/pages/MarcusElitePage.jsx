...
const GraduatesSection = ({ t }) => {
  const data = t.marcus.graduatesSection;
  if (!data) return null;

  return (
    <section className="py-32 bg-[#050505] border-y border-white/5 relative overflow-hidden" id="results">
      <div className="max-w-7xl mx-auto px-8 relative z-10">
        <SectionHeading 
          pre={data.pre}
          title={data.title}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-20">
          {data.items.map((item, i) => (
            <Reveal key={i} delay={i * 0.1}>
              <div className="mv-glass-card p-10 border border-white/10 bg-white/[0.02] hover:border-[#C9A84C]/30 transition-all group h-full flex flex-col">
                {/* Header: Avatar, Name, Bio */}
                <div className="flex items-center gap-5 mb-8">
                  <div className="w-14 h-14 rounded-full border border-[#C9A84C] flex items-center justify-center font-oswald text-lg text-[#C9A84C] font-bold">
                    {item.name.split(' ')[0][0]}{item.name.split(' ')[1]?.[0] || ''}
                  </div>
                  <div className="flex flex-col">
                    <h3 className="font-oswald text-base text-white font-bold tracking-[0.1em] uppercase leading-tight">{item.name}</h3>
                    <p className="font-oswald text-[10px] text-[#6A6A6A] tracking-widest uppercase mt-1">{item.bio}</p>
                  </div>
                </div>
                ...
                <div className="flex gap-1 mb-6">
                  {[...Array(5)].map((_, i) => (
                    <StarIcon key={i} className="text-[#C9A84C]" />
                  ))}
                </div>

                {/* Achievement Highlight */}
                <h4 className="font-oswald text-xl text-[#C9A84C] font-bold leading-tight mb-6 uppercase tracking-tight">
                  "{item.highlight}"
                </h4>

                {/* Review Body */}
                <p className="text-[#A3A3A3] text-sm font-light leading-relaxed italic opacity-80 group-hover:opacity-100 transition-opacity">
                  {item.review}
                </p>
              </div>
            </Reveal>
        ...
        {/* Legal Disclaimer */}
        <p className="mt-20 text-center text-[10px] text-[#444] font-oswald tracking-widest uppercase max-w-3xl mx-auto leading-relaxed italic">
          {data.disclaimer}
        </p>
      </div>
...
const MarcusElitePage = () => {
  ...
  return (
    <div className="bg-[#000000] min-h-screen text-[#F5F5F5] font-['Barlow'] overflow-x-hidden group/main">
      ...
      <main>
        {/* --- Hero Section --- */}
        <section className="relative pt-[180px] lg:pt-[220px] pb-32 overflow-hidden" id="about">
          ...
          <div className="max-w-7xl mx-auto px-8 relative z-10">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-24 items-center">
              <div className="flex flex-col items-center text-center lg:items-start lg:text-left">
                <Reveal>
                  <div className="inline-flex items-center gap-3 border-l-2 border-[#C9A84C] bg-white/[0.03] px-6 py-2 mb-10">
                    <span className="font-oswald text-[#C9A84C] text-[11px] font-bold tracking-[0.4em] uppercase">{t.marcus.heroPreHeader}</span>
                  </div>
                </Reveal>
                
                <Reveal delay={0.1}>
                  <h1 className="font-oswald lg:font-barlow-cond text-[13vw] min-[400px]:text-[64px] md:text-[90px] lg:text-[110px] leading-[0.85] text-white tracking-tighter mb-10 uppercase font-black lg:font-extrabold break-words">
                    {t.marcus.heroTitle.part1}<br/>
                    <span className="lg:mv-text-gold-gradient">{t.marcus.heroTitle.part2}</span><br/>
                    <span className="text-[#C9A84C] lg:text-white">{t.marcus.heroTitle.part3}</span>
                  </h1>
                </Reveal>

                {/* Mobile Subtitle */}
                <Reveal delay={0.2}>
                  <p className="lg:hidden font-oswald text-white/60 text-sm md:text-lg tracking-[0.1em] uppercase italic mb-12 max-w-2xl leading-relaxed">
                ...
                {/* Desktop Bullets */}
                <Reveal delay={0.2}>
                  <div className="hidden lg:flex flex-col gap-5 mb-12">
                ...
                <Reveal delay={0.3}>
                  <div className="flex flex-col items-center lg:items-start gap-8 w-full">
                      ...
                      <button 
                        ...
                        className="w-full sm:w-auto bg-[#C9A84C] text-black px-16 py-6 lg:px-12 lg:py-6 font-oswald text-xl lg:text-lg tracking-widest hover:bg-white transition-all shadow-2xl uppercase font-extrabold mv-glow whitespace-nowrap"
                      >
                        {t.marcus.ctaPrimary}
                      </button>
